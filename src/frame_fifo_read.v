...
begin
	...
	begin
		...
		if(state == S_CHECK_FIFO)
			...
			burst_cnt <= burst_cnt;
		//
		// 修改：设置起始地址为图像最后一行（479行 * 640像素/行 = 306560）
		if(state == S_ACK)
			begin
				if(read_addr_index_d1 == 2'd0) begin
					row_start_addr <= read_addr_0 + 21'd306560;  // 从最后一行开始
					App_rd_addr_r <= read_addr_0 + 21'd306560;
				end
				else if(read_addr_index_d1 == 2'd1) begin
					row_start_addr <= read_addr_1 + 21'd306560;
					App_rd_addr_r <= read_addr_1 + 21'd306560;
				end
				else if(read_addr_index_d1 == 2'd2) begin
					row_start_addr <= read_addr_2 + 21'd306560;
					App_rd_addr_r <= read_addr_2 + 21'd306560;
				end
				else if(read_addr_index_d1 == 2'd3) begin
					row_start_addr <= read_addr_3 + 21'd306560;
					App_rd_addr_r <= read_addr_3 + 21'd306560;
				end
				col_cnt <= 10'd0;
			end
		// 修改：按行反向读取（每行内正序，行间倒序）
		else if(App_rd_en)
			begin
				if(col_cnt < IMG_WIDTH - 1) begin
					...
					// 读完一行，跳到前一行
					row_start_addr <= row_start_addr - IMG_WIDTH;
					App_rd_addr_r <= row_start_addr - IMG_WIDTH;
					col_cnt <= 10'd0;
				end
			end
		else
